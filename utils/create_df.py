    ...
    for filename in os.listdir(dir_path):
        if filename.endswith(".txt"):
            ...
            with open(file_path, "r") as f:
                ...
                print(f"Header for {filename}: {header}")
                ...
                try:
                    ...
                    print(f"Error in finding required columns in {filename}: {str(e)}")
                    continue
                ...
                for line in lines[1:]:
                    ...
                    print(f"Processing line in {filename}: {cols}")

                    # Skip lines that do not have the expected number of columns
                    if len(cols) <= max(
                        sample_name_idx,
                        percent_area_idx,
                        area_idx if area_idx is not None else -1,
                    ):
                        print(
                            f"Skipping line in {filename}: {cols}, insufficient columns"
                        )
                        continue
                    ...
                    try:
                        ...
                        print(
                            f"Index error for line in {filename}: {cols}, error: {str(e)}"
                        )
                        continue
                ...
                for sample_name, data in sample_data.items():
                    ...
                    areas = (
                        [val for val in data["areas"]] if area_idx is not None else []
                    )
